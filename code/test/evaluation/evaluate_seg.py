...
def parse_args():
    ...
    parser.add_argument('task', choices=['seg', 'det', 'drivable', 'lane'])
...
def evaluate_segmentation(gt_dir, result_dir, num_classes):
    ...
    gt_dict = dict([(osp.split(p)[1][:], p)
                    for p in find_all_png(gt_dir) if osp.split(p)[1][:] in result_dict])
...
def evaluate_lane(gt_dir, result_dir, num_classes=2):
    result_dict = dict([(osp.split(p)[1][:], p)
                        for p in find_all_png(result_dir)])
    gt_dict = dict([(osp.split(p)[1][:], p)
                    for p in find_all_png(gt_dir) if osp.split(p)[1][:] in result_dict])
