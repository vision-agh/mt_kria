...
class ConvertFromInts(object):
    def __init__(self, seg_type=np.int64):
        self.seg_type = seg_type

    def __call__(self, image, seg=None):
        return image.astype(np.float32), seg.astype(self.seg_type)


class SubtractMeans(object):
    def __init__(self, mean, seg_type=np.int64):
        self.seg_type = seg_type
    ...
    def __call__(self, image, seg):
        ...
        if prandom.uniform(0, 1) >= 0.7:
            image = am.add_sun_flare(image, no_of_flare_circles=prandom.choice([1, 2, 3, 4]),
                                     src_radius=prandom.randint(300, 400))
        image = image.astype('float32')
        return image, seg
...
class PhotometricDistort(object):
    ...
    def __call__(self, image, seg):
        ...
        # im, seg = self.rand_light_noise(im, seg)
        im = im.astype('uint8')
...
class RandomRotate(object):
    def __init__(self, degree=1.0, seg_type=np.int64):
        self.seg_type = seg_type
        self.degree = degree

    def __call__(self, image, seg):
        random_angle = (random.random() - 0.5) * 2 * self.degree
        if self.seg_type == np.int64:
            seg = seg.astype(np.int32)
        else:
            seg = seg.astype(self.seg_type)
...
class LaneNpyAugmentation(object):
    def __init__(self, size, mean):
        self.mean = mean
        self.size = size
        self.augment = Compose([
            ConvertFromInts(seg_type=np.float32),
            PhotometricDistort(),
            RandomRotate(seg_type=np.float32),
            RandomMirror(),
            Resize(self.size),
            SubtractMeans(self.mean, seg_type=np.float32)
        ])
